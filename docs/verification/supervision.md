...
| Antigravity CLI | 1.1.19 | Plugin hooks `PreInvocation` / `Stop` | `PreInvocation` opened the launch and steered turns, and `Stop` closed the execution loop and touched the turn-end notification. The delivery acknowledgement separately uses only the live `esc to cancel` footer. |
