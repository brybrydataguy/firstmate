...
| Antigravity CLI | 1.1.19 | Plugin hooks `PreInvocation` / `Stop` plus read-only conversation task rows | `PreInvocation` opened the launch and steered turns. A running background command appeared as a `steps` row with status 2 and task details, while completed rows moved to status 3. The synchronous Stop observer waits on that transition, then closes semantic state and touches the turn-end notification without relying on capped continuation retries. The delivery acknowledgement separately uses only the live `esc to cancel` footer. |
