...
After a host reboot, a scope that can be proved to predate the current boot is retired without signaling; [`bin/fm-task-process-lib.sh`](../bin/fm-task-process-lib.sh) owns that recovery.
