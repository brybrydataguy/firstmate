...
After a host reboot, a trustworthy active scope whose recorded boot generation differs from the current host generation can be retired without signaling; generation-less records cannot use this recovery and retain the ordinary identity and ownership checks owned by [`bin/fm-task-process-lib.sh`](../bin/fm-task-process-lib.sh).
