...
After a host reboot, a trustworthy active scope whose recorded boot generation differs from the current host generation can be retired without signaling; older version 2 Darwin scopes can also recover only when corroborated legacy evidence proves they predate the boot, while unavailable or ambiguous evidence retains the ordinary fail-closed checks owned by [`bin/fm-task-process-lib.sh`](../bin/fm-task-process-lib.sh).
