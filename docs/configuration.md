...
After a host reboot, a scope whose recorded boot generation differs from the current host generation is retired without signaling; generation-less records remain refused, and [`bin/fm-task-process-lib.sh`](../bin/fm-task-process-lib.sh) owns that recovery.
